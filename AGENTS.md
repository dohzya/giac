...
- **Default**: English (US) everywhere (identifiers, comments, docs, logs,
  APIs).
- Domain/business terms in other languages are allowed **only** if listed in a
  `GLOSSARY.md` file.
  - If a term is not in the glossary → use an English equivalent.
- Treat accepted non-English terms as **atomic tokens** (`SIRET`, `facture`),
  but keep surrounding words in English (`factureId`, `parse_siret` - but
  respect the naming conventions of the chosen stack or project.).
