...
Specification–driven prompt builder for composing an AI agent "style" across
five independent behavioral axes. GIAC loads a structured YAML spec
(`spec.yml`), lets you define / interactively fill a profile (levels 0–10 per
axis), and generates a reproducible multi-fragment prompt.
...
Each axis has 11 discrete levels (0–10) defined in `spec.yml` with French &
English names plus prompt fragments. The final prompt is built by selecting one
level per axis and applying arbitration rules (higher-priority axes override
lower ones when fragments clash).
...
The specification object (`Spec`) aggregates all axes plus global description
and introductory prompt fragments.
...
The code uses permission sets (see `deno.jsonc`). Avoid running with `-A`; use
tasks.
...
French inside internal identifiers; public output matches selected language.

## 8. Glossary (French Domain Terms)

See `GLOSSARY.md` for canonical definitions. Terms are atomic in code (e.g.
`telisme`) to preserve domain semantics.

## 9. Error Handling & Validation

- Specification validation uses Zod (literal unions for levels) with custom
  error classes (`SpecValidationError`, etc.).
- Non-null assertions removed; explicit checks ensure safety with
...
Permissions: tasks apply predefined sets; extend `deno.jsonc` instead of using
`-A`.

## 13. Contributing

1. Open an issue describing the change
2. Keep French domain vocabulary limited to glossary terms; everything else in
   English
...
No license file provided. Add one (e.g., MIT) if external distribution is
intended.

## Acknowledgements

Built with Deno 2, Zod, and a strict TypeScript setup. Inspired by structured
